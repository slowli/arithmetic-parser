...
fn widening_type_hint_with_slice_arg() {
    ...
    assert_eq!(type_env["foo"].to_string(), "fn([Num; N]) -> [Num; N]");
}
...
fn unifying_tuples_with_middle() {
    ...
    assert_eq!(type_env["ys"].to_string(), "(Num, Num, fn('T) -> 'T)");
